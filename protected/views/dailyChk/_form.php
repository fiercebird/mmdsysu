...
<div class="form" >
    ...
    <?php Yii::app()->clientScript->registerScript('reset', '
		$(document).ready(function(){
		
		$("#DailyChk_classId").html("");
		//$("#DailyChk_bid").children(":nth-child(1)").attr("value", " ");
		});
		'); ?>
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="table-layout:fixed;width:100%;">
        ...
        <tr>
            <!-- 
                <td width="5%">校区</td>
                <td width="10%"><?php
            /*    echo $form->dropDownList($model, 'cid', Campus::model()->getCampusList(), array(
              'ajax' => array(
              'type' => 'POST',
              'url' => Yii::app()->createUrl('building/getAjaxBuilding'),
              'update' => '#DailyChk_bid',
              'data' => array(Yii::app()->request->csrfTokenName => Yii::app()->request->getCsrfToken(), 'campus' => 'js:$("#DailyChk_cid").val()'),
              'success' => 'function(data){$("#DailyChk_bid").html(data); $("#DailyChk_classId").html("");}',
              ),
              )
              ); */
            ?></td> -->
            <td width="5%">校区</td><td width="10%"><?php
                echo $form->dropDownList($model, 'bid', Building::model()->getBuildingList(), array(
                    'ajax' => array(
                        'type' => 'POST',
                        'url' => Yii::app()->createUrl('classroom/getAjaxClassroom'),
                        'update' => '#DailyChk_classId',
                        'data' => array(Yii::app()->request->csrfTokenName => Yii::app()->request->getCsrfToken(), 'classroom' => 'js:$("#DailyChk_bid").val()')
                    ),
                    'empty' => '请选择校区',
                        )
                );
                ?></td>
            <td width="6%">记录课室</td>
            <td width="10%"><?php echo $form->dropDownList($model, 'classId', array()); ?></td>
            <td width="6%">记录周次</td>
...
1.按下中控面版按键能否正常切换
2.用摇控器检查下投影灯泡使用时间。并站在教室中后部（教室的正中间靠后一点点的部位）用肉眼观察投影内容是否清晰可见
3.如有电动幕、或手动幕的课室，要检查投影幕布升降有无异常，有无泛黄、画花等
4.检查有线、无线麦能否正常使用，声音有无断续。（要熟悉麦到混音器的连接，可调频无线麦的使用等）
5.调混音器或功放使电脑、麦的音量在较理想的位置后，记住该数值，以后保持该数值不变。（要熟悉混音设备的使用）
6.多媒体讲台有无损坏，柜门有无损坏等
7.麦头的防风罩旧了、坏了、掉了、要及时补上
8.做好多媒体设备的清洁工作，做到每月至少清洁一次。（很重要！！！）
                        </textarea></td>
                </tr>
            </table>
        </div>
    ...
    <fieldset>
        ...
        <table cellpadding="10" width="37%" style="float: left;">
            <tr>
                <td>
