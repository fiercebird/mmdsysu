...
$this->menu=array(
	array('label'=>'添加记录', 'url'=>array('create')),
		array('label' => '检索无线麦', 'url' => '#',  'linkOptions' => array('class' => 'search-button'), ),
);
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl . "/js/getMnlRepairDetail.js");
...
$('.search-form form').submit(function(){
	$('#mai-no-line-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<table id="bool_display_tbl">
    <tr>
        ...
        <td>
            购买时间
            <?php echo CHtml::activeCheckBox($model, 'deviceNum', array('onclick'=>'chkBox(this, 10);'))?>
	</td>
        <td>
            供应商
            <?php echo CHtml::activeCheckBox($model, 'guarentee', array('onclick'=>'chkBox(this, 11);'))?>
	</td>
        <td>
            保修情况
            <?php echo CHtml::activeCheckBox($model, 'buyTime', array('onclick'=>'chkBox(this, 12);'))?>
	</td>
    </tr>
</table>
...
_JS_;
	...
	'columns'=>array(
		...
		'isUsing',
                ...
                array(
                    'name' => '维修次数',
                    'value' => 'MnlRepair::model()->getRepair($data->mid)',
                    'type' => 'html'
                ),
		array(
			'class'=>'CButtonColumn',
...
<?php //echo CHtml::link('检索无线麦','#',array('class'=>'search-button')); ?>
</div>
