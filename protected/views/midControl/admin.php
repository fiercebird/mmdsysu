...
$this->menu=array(
	...
	array('label' => '检索中控', 'url' => '#',  'linkOptions' => array('class' => 'search-button'), ),
);
Yii::app()->clientScript->registerScriptFile(Yii::app()->request->baseUrl . "/js/getMidRepairDetail.js");
...
$('.search-form form').submit(function(){
	$('#mid-control-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<table id="bool_display_tbl">
    <tr>
        ...
        <td>
            是否网络中控
            <?php echo CHtml::activeCheckBox($model, 'buyTime', array('onclick'=>'chkBox(this, 7);'))?>
	</td>
        <td>
            购买时间
            <?php echo CHtml::activeCheckBox($model, 'isUsing', array('onclick'=>'chkBox(this, 10);'))?>
	</td>
        <td>
            保修情况
...
_JS_;
	...
	'columns'=>array(
		...
		'isUsing',
                ...
                array(
                    'name' => '维修次数',
                    'value' => 'MidRepair::model()->getRepair($data->mid)',
                    'type' => 'html'
                ),
		array(
			'class'=>'CButtonColumn',
...
<?php //echo CHtml::link('检索中控','#',array('class'=>'search-button')); ?>
</div>
