...
class SoundControlController extends Controller
{
    ...
    {
        $model = new SoundControl;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['SoundControl']))
        {
            $model->attributes = $_POST['SoundControl'];
            if ($model->save())
        ...
        if (isset($_FILES['batchFile']) && $_FILES['batchFile']['error'] == 0)
        {
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++)
            {
                ...
                else
                {
                    ...
                    $model->road = $data->val($i, 7);
                    $model->price = $data->val($i, 8);
                    $model->supplyCompany = $data->val($i, 9);
                    $model->guarentee = $data->val($i, 10);
                    $model->buyTime = $data->val($i, 11);
    ...
    {
        $dataProvider = new CActiveDataProvider('SoundControl');
    ...
    {
        $model = new SoundControl('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['SoundControl'])) {
            $model->attributes = $_GET['SoundControl'];
            $_SESSION['prevAttr'] = $_GET['SoundControl'];
        }
     ...
     * @return SoundControl the loaded model
     * @throws CHttpException
     */
    public function loadModel($id)
    {
        $model = SoundControl::model()->findByPk($id);
        if ($model === null)
     ...
     * @param SoundControl $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'sound-control-form')
        {
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '设备号', '品牌', '型号', '输入路数', '价格', '供应商', '购买时间', '在用/备用', '保修情况');
        
        //从session中读取查询条件
        $criteria = $_SESSION['cc'];
        
        $amplifier = SoundControl::model()->findAll($criteria);
        ...
        $xls = new JPhpExcel('UTF-8', false, '调音台');
        $xls->addArray($data);
        $xls->generateXML('调音台', false);
    }
    
    //获取维修的详情
    public function actionGetRepairDetails($sid)
    {
        $res = SouRepair::model()->findAllByAttributes(array('sid' => $sid));
        $output['res'] = array();
