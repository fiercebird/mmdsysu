...
class MaiHasLineController extends Controller
{
    ...
    {
        $model = new MaiHasLine;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['MaiHasLine']))
        {
            $model->attributes = $_POST['MaiHasLine'];
            if ($model->save())
    ...
    {
        $dataProvider = new CActiveDataProvider('MaiHasLine');
    ...
    {
        $model = new MaiHasLine('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['MaiHasLine'])) {
            $model->attributes = $_GET['MaiHasLine'];
            $_SESSION['prevAttr'] = $_GET['MaiHasLine'];
        }
     ...
     * @return MaiHasLine the loaded model
     * @throws CHttpException
     */
    public function loadModel($id)
    {
        $model = MaiHasLine::model()->findByPk($id);
        if ($model === null)
     ...
     * @param MaiHasLine $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'mai-has-line-form')
        {
    ...
    //获取维修的详情
    public function actionGetRepairDetails($mid)
    {
        $res = MhlRepair::model()->findAllByAttributes(array('mid' => $mid));
        $output['res'] = array();
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '品牌', '设备号', '型号', '价格',  '供应商', '保修情况', '购买时间', '在用/备用');
        
        //从session中读取查询条件
        $criteria = $_SESSION['cc'];
        
        $amplifier = Amplifier::model()->findAll($criteria);
        ...
        $xls = new JPhpExcel('UTF-8', false, '有线麦');
        $xls->addArray($data);
        $xls->generateXML('有线麦', false);
    }
