...
class ComputerController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Computer']))
        {
            ...
            if ($model->save()) {               
                $this->redirect(array('view', 'id' => $model->comid));
            }
        }
        ...
        if(isset($_FILES['batchFile']) && $_FILES['batchFile']['error']==0)
        {
            ...
            for($i=2;$i<=$data->sheets[0]['numRows'];$i++)
            {
                ...
                else
                {
                    ...
                    $model->cpu = $data->val($i, 11);
                    $model->memory = $data->val($i, 12);
                    $model->hardDisk = $data->val($i, 13);
                    $model->displayer = $data->val($i, 14);
                    $model->mac = $data->val($i, 15);
                    $model->isUsing = $data->val($i, 16);
                    $model->save();
                    $num++;
                }   
            }
    ...
    public function actionAdmin()
    {
        ...
        if (isset($_GET['Computer'])) {
            ...
            $_SESSION['prevAttr'] = $_GET['Computer'];
        }
    ...
    //获取维修的详情
    public function actionGetRepairDetails($comid)
    {
        $res = ComRepair::model()->findAllByAttributes(array('comid' => $comid));
        $output['res'] = array();
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '设备号', '类型', '购买时间', '保修情况', '供应商', '价格', '显示器类型', 'CPU', '品牌', '内存', '硬盘', '在用/备用', '物理地址', 'IP');
        
        //从session中读取查询条件
        $criteria = $_SESSION['cc'];
        
        $computers = Computer::model()->findAll($criteria);
        foreach ($computers as $value)
        {
            ...
            array_push($tmp, $value->displayer);
            array_push($tmp, $value->cpu);
            array_push($tmp, $value->brand);
            array_push($tmp, $value->memory);
            array_push($tmp, $value->hardDisk);
            array_push($tmp, $value->isUsing);
            array_push($tmp, $value->mac);
            array_push($tmp, $value->ip);
            array_push($data, $tmp);
        }

        Yii::import('application.extensions.phpexcel.JPhpExcel');
        $xls = new JPhpExcel('UTF-8', false, '电脑');
        $xls->addArray($data);
        $xls->generateXML('电脑', false);
    }
