...
class ProScreenController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['ProScreen']))
        {
            $model->attributes = $_POST['ProScreen'];
            if ($model->save())
        ...
        if (isset($_FILES['batchFile']) && $_FILES['batchFile']['error'] == 0)
        {
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++)
            {
                ...
                else
                {
                    ...
                    $model->buyTime = $data->val($i, 9);
                    $model->size = $data->val($i, 10);
                    $model->isAuto = $data->val($i, 11);
                    $model->what = $data->val($i, 12);
                    $model->isUsing = $data->val($i, 13);
                    $model->save();
                    $num++;
                }
            }
    ...
    {
        $dataProvider = new CActiveDataProvider('ProScreen');
    ...
    {
        $model = new ProScreen('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['ProScreen'])) {
            $model->attributes = $_GET['ProScreen'];
            $_SESSION['prevAttr'] = $_GET['ProScreen'];
        }
     ...
     * @return ProScreen the loaded model
     * @throws CHttpException
     */
    public function loadModel($id)
    {
        $model = ProScreen::model()->findByPk($id);
        if ($model === null)
     ...
     * @param ProScreen $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'pro-screen-form')
        {
    ...
    {
        $res = PsRepair::model()->findAllByAttributes(array('pid'=>$pid));
        $output['res'] = array();
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '设备号', '品牌', '型号', '尺寸', '价格', '供应商', '购买时间', '在用/备用', '手动/电动', '白塑/玻珠', '保修情况');
        
        
        //从session中读取查询条件
        $criteria = $_SESSION['cc'];
        $lamp = ProScreen::model()->findAll($criteria);
        foreach($lamp as $value)
        {
            ...
            array_push($tmp, $value->isAuto);
            array_push($tmp, $value->what);
            array_push($tmp, $value->guarentee);
            array_push($data, $tmp);
        }
        Yii::import('application.extensions.phpexcel.JPhpExcel');
        $xls = new JPhpExcel('UTF-8', false, '投影幕');
        $xls->addArray($data);
        $xls->generateXML('投影幕', false);
    }
}
