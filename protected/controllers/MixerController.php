...
class MixerController extends Controller
{
    ...
    {
        $dataProvider = new CActiveDataProvider('Mixer');
    ...
    {
        $model = new Mixer('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Mixer'])) {
            $model->attributes = $_GET['Mixer'];
            $_SESSION['prevAttr'] = $_GET['Mixer'];
        }
     ...
     * @return Mixer the loaded model
     * @throws CHttpException
     */
    public function loadModel($id)
    {
        $model = Mixer::model()->findByPk($id);
        if ($model === null)
     ...
     * @param Mixer $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'mixer-form')
        {
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '设备号', '品牌', '功率','购买时间', '供应商', '型号', '价格', '在用/备用', '保修情况');
        
        //从session中读取查询条件
        $criteria = $_SESSION['cc'];
        
        $amplifier = Mixer::model()->findAll($criteria);
        ...
        $xls = new JPhpExcel('UTF-8', false, '混音器');
        $xls->addArray($data);
        $xls->generateXML('混音器', false);
    }
    
    //获取维修的详情
    public function actionGetRepairDetails($mid)
    {
        $res = MixRepair::model()->findAllByAttributes(array('mid'=>$mid));
        $output['res'] = array();
