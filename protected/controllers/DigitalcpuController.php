...
class DigitalcpuController extends Controller
{
	/**
	...
	{
		$model=new Digitalcpu;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['Digitalcpu']))
		{
			$model->attributes=$_POST['Digitalcpu'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('Digitalcpu');
	...
	{
            $model = new Digitalcpu('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Digitalcpu'])) {
            $model->attributes = $_GET['Digitalcpu'];
            $_SESSION['prevAttr'] = $_GET['Digitalcpu'];
        }
	 ...
	 * @return Digitalcpu the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Digitalcpu::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Digitalcpu $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='digitalcpu-form')
		{
	...
	public function actionExcel()
    {
        ...
        $amplifier = Digitalcpu::model()->findAll($criteria);
        ...
        $xls = new JPhpExcel('UTF-8', false, '数字处理器');
        $xls->addArray($data);
        $xls->generateXML('数字处理器', false);
    }
    
    //获取维修的详情
    public function actionGetRepairDetails($sid)
    {
        $res = DigRepair::model()->findAllByAttributes(array('sid' => $sid));
        $output['res'] = array();
