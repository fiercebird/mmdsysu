...
class AmplifierController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Amplifier'])) {
            $model->attributes = $_POST['Amplifier'];
            if ($model->save())
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('Amplifier');
    ...
    public function actionAdmin() {
        $model = new Amplifier('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Amplifier'])) {
            $model->attributes = $_GET['Amplifier'];
            $_SESSION['prevAttr'] = $_GET['Amplifier'];
        }
    ...
    //获取维修的详情
    public function actionGetRepairDetails($aid)
    {
        $res = AmpRepair::model()->findAllByAttributes(array('aid' => $aid));
        $output['res'] = array();
     ...
     * @return Amplifier the loaded model
     * @throws CHttpException
     */
    public function loadModel($id) {
        $model = Amplifier::model()->findByPk($id);
        if ($model === null)
     ...
     * @param Amplifier $model the model to be validated
     */
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'amplifier-form') {
    ...
    public function actionExcel()
    {
        $data = array();
        //header
        $data[1] = array('校区', '教学楼', '课室', '设备号', '品牌', '型号', '功率', '价格', '在用/备用', '购买时间', '供应商', '保修情况');
        ...
        $xls = new JPhpExcel('UTF-8', false, '功放');
        $xls->addArray($data);
        $xls->generateXML('功放', false);
    }
}
